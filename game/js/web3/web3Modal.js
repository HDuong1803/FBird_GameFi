...
async function fetchAccountData() {
  ...
  const cctBalance = await getCCTBalance(accInfo.walletAddress);
  accInfo.cctBalance = cctBalance;
  ...
  accountContainer.innerHTML = `
    ...
    <div class='line'>
      <span>CCT: </span>
      <span>${parseFloat(accInfo.cctBalance).toFixed(2)}</span>
    </div>
    <div class='line'>
      <span>Ticket: </span>
...
async function deposit(address, amount) {
  const chocoContract = new web3.eth.Contract(Choco_ABI, Choco_ADDRESS);
  await chocoContract.methods
...
async function getCCTBalance(address) {
  const chocoContract = new web3.eth.Contract(Choco_ABI, Choco_ADDRESS);
  const balance = await chocoContract.methods.balanceOf(address).call();
